...
@Component({
  selector: 'app-managejobseeker',
  templateUrl: './managejobseeker.component.html',
  styleUrls: ['./managejobseeker.component.scss']
})
export class ManagejobseekerComponent implements OnInit {
  ...
  profile: SiteModel.CreateProfile;
  ...
  GetUserByType = (): void => {
    this.masterdata.getUserByType('JOB_SEEKER').subscribe(result => {
      if (result) {
        this.users = result;
  ...
  editUser = (template: TemplateRef<any>, user: SiteModel.User): void => {
    ...
    // editProfile = (profile: SiteModel.CreateProfile): void => {
    //   debugger;
  ...
  updateUser = (): void=> {
    this.managejobseekerService.updateCurrentUser(this.id,this.user).subscribe(result => {
      if (result) {
        this.GetUserByType();
  ...
  editProfile = (user: SiteModel.User): void => {
    debugger;
    this.user = new SiteModel.UpdateCurrentUser();
    this.id = user.id;
    this.router.navigate(['/job-seeker/profile', this.id]);
  }
}
